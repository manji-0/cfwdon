...
#[test]
fn compatibility_terms_of_service_fallback_shape_is_stable() {
    let value = build_default_terms_of_service_document("terms body");
    assert_has_pointer(&value, "/effective_date");
    assert_eq!(value.pointer("/effective"), Some(&serde_json::json!(true)));
    assert_eq!(
        value.pointer("/content").and_then(|v| v.as_str()),
        Some("<p>terms body</p>")
    );
