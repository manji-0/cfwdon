...
use crate::scheduled_statuses::{
    delete_scheduled_status_response, process_due_scheduled_statuses, scheduled_status_response,
    scheduled_statuses_response, update_scheduled_status_response,
};
use worker::Router;
...
pub(crate) fn add_poll_routes(router: Router<'static, ()>) -> Router<'static, ()> {
    router
        ...
        .post_async(
            ...
            |req, ctx| async move { process_due_scheduled_statuses(req, ctx).await },
        )
