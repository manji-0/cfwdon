...
use crate::{
    ...
    find_status_poll_by_status_id, is_public_activitypub_visibility, load_in_reply_to_account_id,
    parse_lookup_handle, resolve_account_reference, resolve_remote_status_by_url,
    search_local_status_rows, search_remote_status_rows, search_text_match_rank, strip_html_tags,
};
...
pub(crate) fn parse_status_search_query(query: &str) -> ParsedStatusSearchQuery {
    ...
    for token in tokenize_status_search_query(query) {
        ...
        if let Some(value) = token.strip_prefix("before:") {
            parsed.before = earlier_status_search_bound(
                parsed.before.take(),
                normalize_status_search_timestamp(value),
            );
            continue;
        }
        if let Some(value) = token.strip_prefix("after:") {
            parsed.after = later_status_search_bound(
                parsed.after.take(),
                normalize_status_search_timestamp(value),
            );
            continue;
        }
        if let Some(value) = token.strip_prefix("during:") {
            parsed.after = later_status_search_bound(
                parsed.after.take(),
                normalize_status_search_timestamp(value),
            );
            parsed.before = earlier_status_search_bound(
                parsed.before.take(),
                next_day_status_search_timestamp(value),
            );
            continue;
        }
        if let Some(value) = token.strip_prefix("language:") {
            let value = unquote_status_search_token(value).to_ascii_lowercase();
        ...
        if let Some(value) = token.strip_prefix("is:") {
            match unquote_status_search_token(value)
                ...
                .as_str()
            {
                "reply" => parsed.is_reply = Some(!negated),
                "sensitive" => parsed.is_sensitive = Some(!negated),
                _ => {}
            }
            continue;
        }
        if let Some(value) = token.strip_prefix("has:") {
            match unquote_status_search_token(value)
                ...
                .as_str()
            {
                "media" => parsed.has_media = Some(!negated),
                "poll" => parsed.has_poll = Some(!negated),
                "embed" => parsed.has_embed = Some(!negated),
                _ => {}
            }
            continue;
        }
        terms.push(unquote_status_search_token(&token));
    }
...
pub(crate) fn status_search_rank(query: &str, content: &str, spoiler_text: &str) -> (u8, u8) {
    [
        (search_text_match_rank(query, content), 0u8),
        (search_text_match_rank(query, spoiler_text), 1u8),
    ]
    .into_iter()
    .min()
    .unwrap_or((3, 1))
...
pub(crate) async fn search_statuses_for_v2(
    ...
    let search_text = parsed_query.text_query.as_str();
    ...
    if !matches!(
        ...
        .await?
        {
            ...
            entries.push((
                (
                    status_search_rank(search_text, &status._text_content, &status.spoiler_text),
