...
COMPAT_GAPS = {
    ("GET", "/oauth/userinfo"): "minimal OAuth userinfo claims",
    ("POST", "/oauth/userinfo"): "minimal OAuth userinfo claims",
    ("GET", "/api/v1/apps/verify_credentials"): "response shape は寄せたが bearer application token の検証は未実装",
    ("POST", "/api/v1/emails/confirmations"): "auth gate は入れたが mail dispatch / application ownership 条件は未実装",
    ("GET", "/api/v1/emails/check_confirmation"): "boolean response と auth gate は入れたが confirmation state は未実装",
    ("GET", "/api/v1/timelines/link"): "discoverable public statuses を返すが trending 判定は未実装",
    ("GET", "/api/v1/streaming"): "placeholder SSE endpoint",
    ("GET", "/api/v1/streaming/(*any)"): "placeholder SSE endpoint",
    ("POST", "/api/v1/statuses/:id/quotes/:id/revoke"): "local quote の関連解除のみ実装",
    ("POST", "/api/v1/accounts"): "placeholder account registration response",
    ("POST", "/api/v1/accounts/:id/remove_from_followers"): "local/remote follower の切断のみ実装",
    ("GET", "/api/v1/timelines/home"): "followed hashtag 混在は反映したが upstream の access control settings 差分は残る",
    ("GET", "/api/v1/timelines/tag/:hashtag"): "tag filter / local-remote 混在は実装したが public preview access control settings は未対応",
    ("GET", "/api/v1/timelines/tag/:id"): "tag filter / local-remote 混在は実装したが public preview access control settings は未対応",
    ("GET", "/api/v2/search"): "URL / hashtag resolve の詰めが残る",
    ("GET", "/api/v1/statuses/:id/context"): "混在 thread の traversal は改善したが unauthenticated limit / async refresh header は未実装",
    ("GET", "/api/v1/polls/:id"): "permissions / remote poll 精度",
    ("POST", "/api/v1/polls/:id/votes"): "local / remote vote の詰めが残る",
    ("PATCH", "/api/v1/accounts/update_credentials"): "profile / AP update の詰めが残る",
    ("GET", "/api/v1/directory"): "remote discoverable account は混在するが ordering 精度は近似",
    ("GET", "/api/v1/accounts/:id/endorsements"): "local account owner の featured profiles を返すが remote account の featured collection は未対応",
    ...
    ("PUT", "/api/v1/statuses/:id/interaction_policy"): "auth gate と param validation は入れたが quote policy 永続化は未実装",
    ("PATCH", "/api/v1/statuses/:id/interaction_policy"): "auth gate と param validation は入れたが quote policy 永続化は未実装",
    ("POST", "/api/v1/statuses/:id/translate"): "auth gate と response shape は寄せたが翻訳 provider 連携は未実装",
    ("GET", "/api/v1/scheduled_statuses"): "auth gate はあるが永続化された scheduled status 一覧は未実装",
    ("GET", "/api/v1/scheduled_statuses/:id"): "entity shape は寄せたが永続化と ownership 404 は未実装",
    ("PUT", "/api/v1/scheduled_statuses/:id"): "entity shape は寄せたが scheduled_at 更新 semantics は未実装",
    ("PATCH", "/api/v1/scheduled_statuses/:id"): "entity shape は寄せたが scheduled_at 更新 semantics は未実装",
    ("DELETE", "/api/v1/scheduled_statuses/:id"): "auth gate はあるが scheduled status 削除 semantics は未実装",
}

TODO_COMPAT_NOTES = {
    ("GET", "/oauth/userinfo"): "OAuth scope と claims の定義を upstream に寄せる。",
    ("POST", "/oauth/userinfo"): "OAuth scope と claims の定義を upstream に寄せる。",
    ("GET", "/api/v1/apps/verify_credentials"): "response shape は寄せたので、bearer application token の検証を upstream に寄せる。",
    ("POST", "/api/v1/emails/confirmations"): "auth gate は入ったので、mail dispatch 条件と application ownership 条件を upstream に寄せる。",
    ("GET", "/api/v1/emails/check_confirmation"): "boolean response と auth gate は入ったので、email confirmation 状態判定を upstream に寄せる。",
    ("GET", "/api/v1/timelines/link"): "link timeline の trending 判定と article 単位の抽出精度を upstream に寄せる。",
    ...
    ("POST", "/api/v1/statuses/:id/quotes/:id/revoke"): "quote revoke の remote 連携と response semantics を upstream に寄せる。",
    ("POST", "/api/v1/accounts"): "account registration / approval / token 発行の挙動を upstream に寄せる。",
    ("POST", "/api/v1/accounts/:id/remove_from_followers"): "follower removal の federation semantics を upstream に寄せる。",
    ("GET", "/api/v1/timelines/home"): "followed hashtag 混在、sorting、cursor pagination は入ったので、access control settings 差分を upstream に寄せる。",
    ("GET", "/api/v1/timelines/tag/:hashtag"): "tag filter / pagination / remote 混在は入っているので、public preview access control settings を upstream に寄せる。",
    ("GET", "/api/v1/timelines/tag/:id"): "tag filter / pagination / remote 混在は入っているので、public preview access control settings を upstream に寄せる。",
    ("GET", "/api/v2/search"): "`docs/full-todo.md` にある URL resolve / hashtag resolve / ranking の差分をここで追う。",
    ("GET", "/api/v1/statuses/:id/context"): "ancestor / descendant の組み立ては改善済み。未認証時の limit と `Mastodon-Async-Refresh` を upstream に寄せる。",
    ("GET", "/api/v1/polls/:id"): "private / remote permissions と remote poll の扱いを upstream に寄せる。",
    ("POST", "/api/v1/polls/:id/votes"): "local / remote vote の反映、取り消し、再解決の精度を上げる。",
    ("PATCH", "/api/v1/accounts/update_credentials"): "profile fields / media / ActivityPub `Update(Person)` 反映を含めて upstream 挙動に寄せる。",
    ("GET", "/api/v1/directory"): "remote discoverable account は混在したので、ordering と ranking 精度を upstream に寄せる。",
    ("GET", "/api/v1/accounts/:id/endorsements"): "remote account の featured collection 取得を含めて account endorsement 一覧を upstream に寄せる。",
    ...
    ("PUT", "/api/v1/statuses/:id/interaction_policy"): "response shape ではなく、quote policy の保存と response への反映を upstream に寄せる。",
    ("PATCH", "/api/v1/statuses/:id/interaction_policy"): "response shape ではなく、quote policy の保存と response への反映を upstream に寄せる。",
    ("POST", "/api/v1/statuses/:id/translate"): "response shape は寄せたので、翻訳 provider 連携と target language semantics を upstream に寄せる。",
    ("GET", "/api/v1/scheduled_statuses"): "一覧 shape ではなく永続化と pagination を upstream に寄せる。",
    ("GET", "/api/v1/scheduled_statuses/:id"): "detail shape は寄せたので、永続化と ownership 404 を upstream に寄せる。",
    ("PUT", "/api/v1/scheduled_statuses/:id"): "detail shape は寄せたので、scheduled_at 更新 semantics と validation を upstream に寄せる。",
    ("PATCH", "/api/v1/scheduled_statuses/:id"): "detail shape は寄せたので、scheduled_at 更新 semantics と validation を upstream に寄せる。",
    ("DELETE", "/api/v1/scheduled_statuses/:id"): "削除 response shape ではなく、scheduled status delete semantics を upstream に寄せる。",
}
...
def format_compat() -> str:
    grouped: dict[str, list[tuple[tuple[str, str], str]]] = {
        ...
        "Instance / Apps / Trends": [
            (("GET", "/api/v1/apps/verify_credentials"), TODO_COMPAT_NOTES[("GET", "/api/v1/apps/verify_credentials")]),
        ...
        "Timelines / Search": [
            (("GET", "/api/v1/timelines/link"), TODO_COMPAT_NOTES[("GET", "/api/v1/timelines/link")]),
            (("GET", "/api/v1/timelines/home"), TODO_COMPAT_NOTES[("GET", "/api/v1/timelines/home")]),
            (("GET", "/api/v1/timelines/tag/:hashtag"), TODO_COMPAT_NOTES[("GET", "/api/v1/timelines/tag/:hashtag")]),
        ...
        "Statuses / Polls": [
            (("GET", "/api/v1/statuses/:id/context"), TODO_COMPAT_NOTES[("GET", "/api/v1/statuses/:id/context")]),
            (("POST", "/api/v1/statuses/:id/quotes/:id/revoke"), TODO_COMPAT_NOTES[("POST", "/api/v1/statuses/:id/quotes/:id/revoke")]),
            (("GET", "/api/v1/polls/:id"), TODO_COMPAT_NOTES[("GET", "/api/v1/polls/:id")]),
            (("POST", "/api/v1/polls/:id/votes"), TODO_COMPAT_NOTES[("POST", "/api/v1/polls/:id/votes")]),
        ],
        "Accounts / Profile": [
            (("PATCH", "/api/v1/accounts/update_credentials"), TODO_COMPAT_NOTES[("PATCH", "/api/v1/accounts/update_credentials")]),
            (("GET", "/api/v1/directory"), TODO_COMPAT_NOTES[("GET", "/api/v1/directory")]),
        ],
        "Notifications": [
        ],
        ...
        "Accounts / Endorsements": [
            (("GET", "/api/v1/accounts/:id/endorsements"), TODO_COMPAT_NOTES[("GET", "/api/v1/accounts/:id/endorsements")]),
            (("POST", "/api/v1/accounts"), TODO_COMPAT_NOTES[("POST", "/api/v1/accounts")]),
            (("POST", "/api/v1/accounts/:id/remove_from_followers"), TODO_COMPAT_NOTES[("POST", "/api/v1/accounts/:id/remove_from_followers")]),
        ],
        "Push Subscription": [
        ...
        "Scheduled Statuses": [
            ...
            (("DELETE", "/api/v1/scheduled_statuses/:id"), TODO_COMPAT_NOTES[("DELETE", "/api/v1/scheduled_statuses/:id")]),
        ],
    }
